...
function QuizAnalyticsCard({ summary }: { summary: QuizAnalyticsSummary }) {
  ...
  return <section aria-labelledby="quiz-analytics-summary" className="rounded-lg border bg-card p-4 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-mono text-[11px] uppercase tracking-[0.1em] text-primary">Funnel Analytics</p><h2 id="quiz-analytics-summary" className="mt-1 text-lg font-semibold">Resumo externo, somente leitura</h2><p className="mt-1 text-sm leading-relaxed text-muted-foreground">Fonte externa factual; não cria funis nem altera fases, lifecycle ou ofertas do Banco NGV.</p></div><StatusBadge variant={unavailable ? "warning" : summary.source === "UNVERIFIED" ? "neutral" : "success"}>{unavailable ? "Fonte indisponível" : summary.source === "UNVERIFIED" ? "Fonte não verificada" : "Fonte externa"}</StatusBadge></div>
...
function NgvCoreSummaryCard({ summary }: { summary: NgvCoreOperationalSummary }) {
  ...
  const sources = [
    ...
    { key: "nexfy", label: "Nexfy", observedAt: summary.sources.nexfy?.generated_at, values: [["projetos ativos", value(summary.sources.nexfy?.active_projects)], ["produtos ativos", value(summary.sources.nexfy?.active_products)]] },
    { key: "banco_ngv", label: "Banco NGV", observedAt: summary.sources.banco_ngv?.generated_at, values: [["ofertas cadastradas", value(summary.sources.banco_ngv?.offer_tracking_count)], ["registros históricos", value(summary.sources.banco_ngv?.metrics_snapshot_count)]] },
    { key: "quiz_analytics", label: "Funnel Analytics", observedAt: summary.sources.quiz_analytics?.generated_at, values: [["funis", value(summary.sources.quiz_analytics?.project_count)], ["recebendo eventos", value(summary.sources.quiz_analytics?.receiving_events_count)]] },
    ...
    { key: "monitoramento_ngv", label: "Monitoramento", observedAt: summary.sources.monitoramento_ngv?.generated_at, values: [["projetos cadastrados", value(summary.sources.monitoramento_ngv?.projects_total)], ["domínios monitorados", value(summary.sources.monitoramento_ngv?.domains_total)], ["recursos para revisão", value(summary.sources.monitoramento_ngv?.infra_resources_attention)]] },
  ];
